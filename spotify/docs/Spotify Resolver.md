...
description: "Resolve Spotify album links from artist and album names via CLI"
author: riley
category: [Audio, Music]
language: python
path: ".//spotify-resolver.py"
...
A Python CLI tool that searches Spotify's Web API for albums by artist and album name, retrieves the direct Spotify album URL, and automatically copies it to the clipboard. Perfect for quickly finding and sharing album links or integrating into automation workflows like the Deemix downloader.
...
- **Configurable**: Supports config.json for API credentials and search parameters
...
**Spotify API Credentials** - Required in `~/.config/spotify-resolver/config.json`:
```json
{
...
```bash
echo "Metallica - Master of Puppets" | python3 .//spotify-resolver.py
```
...
```bash
# Verbose logging
python3 .//spotify-resolver.py --band "Metallica" --album "Master of Puppets" --verbose

# Print URL instead of copying to clipboard
python3 .//spotify-resolver.py --band "Metallica" --album "Master of Puppets" --no-clipboard

# Custom config file location
python3 .//spotify-resolver.py --band "Metallica" --album "Master of Puppets" --config /path/to/config.json
```

## Script Details

**File Path:** `.//spotify-resolver.py`  
**Language:** Python 3  
**Category:** Audio / Music
...
```bash
python3 .//spotify-resolver.py --band "Pink Floyd" --album "The Wall"
# Output: ✅ Copied to clipboard: Pink Floyd - The Wall
#         https://open.spotify.com/album/5b6s5HYk2p91zQ7q0dMjMF
```

### Example 2: Pipe Input from Echo

```bash
echo "The Beatles - Abbey Road" | python3 .//spotify-resolver.py
# Output: ✅ Copied to clipboard: The Beatles - Abbey Road
#         https://open.spotify.com/album/0wYlP6FVIpNYP0L9FQT6PF
```

### Example 3: Print URL to Console

```bash
python3 .//spotify-resolver.py --band "Radiohead" --album "OK Computer" --no-clipboard
# Output: https://open.spotify.com/album/7eyVLsTbFY40extends8bK (prints to stdout)
```

### Example 4: Use Advanced Query Syntax

```bash
python3 .//spotify-resolver.py --query "artist:Metallica album:Master"
# Output: ✅ Copied to clipboard: Metallica - Master of Puppets
#         https://open.spotify.com/album/2w9zwq3AjxjW2B7D9kZkSeI
```

## Notes

- **Spotify API Credentials Required**: Script will fail if `client_id` and `client_secret` are not configured in `~/.config/spotify-resolver/config.json`
- **Clipboard Fallback**: Uses `pyperclip` library if available, otherwise falls back to `pbcopy` on macOS
- **Logging**: Full logs saved to `~/.local/log/spotify-resolver/spotify-resolver.log` for debugging
- **Rate Limiting**: Spotify API has rate limits; script implements retry logic with exponential backoff
- **Search Accuracy**: Returns the first (most relevant) result from Spotify search; for precise results, ensure exact artist/album names
...
- `Deezer Album Resolver.py.md` - Similar resolver for Deezer music service
