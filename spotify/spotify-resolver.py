...
def load_config(config_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    # Check unified credentials file for Spotify credentials
    if CREDENTIALS_FILE.exists():
        try:
            with open(CREDENTIALS_FILE, 'r') as f:
                creds = json.load(f)
                if 'spotify' in creds:
                    default_config['client_id'] = creds['spotify'].get('client_id')
                    default_config['client_secret'] = creds['spotify'].get('client_secret')
        except (json.JSONDecodeError, IOError) as e:
            logging.warning(f"Error loading credentials from unified config: {e}.")
  ...
  %(prog)s  # Interactive mode
    ...
    try:
        ...
        if not album_id:
            ...
            print("Invalid response from Spotify")
            sys.exit(1)
